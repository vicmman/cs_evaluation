...
sessionId = 2
print(kernel.respond("Hola", sessionId))
while True:
    respuesta_aiml = kernel.respond(input('Paciente>'), sessionId)
    print('Mari:' + respuesta_aiml)

    session_data = kernel.getSessionData()   
    f = io.open(sessionFileName, 'w+')
    f.write("--INICIO--")
    #f.write("Google Said {0}".format(googleTextTranscription))
